...
class ChatClient {
 public:
    explicit ChatClient(const boost::asio::io_service& io_service)
 ...
 private:
    ...
    void handle_write(const boost::system::error_code& error,
        ...
        if (pData != nullptr) {
            PACKET_HEADER* pHeader = reinterpret_cast<PACKET_HEADER*> pData;
    ...
    void handle_receive(const boost::system::error_code& error,
        ...
        if (error) {
            ...
            memcpy(&m_PacketBuffer[m_nPacketBufferMark],
              m_ReceiveBuffer.data(), bytes_transferred);
            ...
            while (nPacketData > 0) {
                ...
                PACKET_HEADER *pHeader =
                    reinterpret_cast<PACKET_HEADER*> &m_PacketBuffer[nReadData];
    ...
    void ProcessPacket(const char *pData) {
        ...
        switch (pheader->nID) {
        case RES_IN:
            PKT_RES_IN *pPacket = reinterpret_cast<PKT_RES_IN*> pData;

            LoginOK();

            std::cout <<
                "클라이언트 로그인 성공 ?: " << pPacket->bIsSuccess
            << std::endl;

            break;
        case NOTICE_CHAT:
            PKT_NOTICE_CHAT* pPacket = reinterpret_cast<PKT_NOTICE_CHAT*> pData;

            std::cout <<
                pPacket->szName << ": " << pPacket->szMessage
            << std::endl;
            break;
        }
    }
...
int main() {
    ...
    while (std::cin.getline(szInputMessage, MAX_MESSAGE_LEN)) {
        ...
        if (Cliet.IsLogin() == false) {
            PKT_REQ_IN SendPkt;
            SendPkt.Init();
            strncpy_s(SendPkt.szName,
              MAX_NAME_LEN, szInputMessage,
              MAX_NAME_LEN - 1);

            Cliet.PostSend(false, SendPkt.nSize,
              reinterpret_cast<char*> &SendPkt);
        } else {
            PKT_REQ_CHAT SendPkt;
            SendPkt.Init();
            strncpy_s(SendPkt.szMessage,
              MAX_MESSAGE_LEN, szInputMessage,
              MAX_MESSAGE_LEN - 1);
